  ...
  async getProducts() {
    ...
    if (archivoexiste) {
      ...
      const productosvisibles = this.#products.filter((elemento) => elemento.status == true);
        ...
        console.log(
            "Estos son los productos actualmente", productosvisibles
            )
        }
        else { console.error(
            "Actualmente no cuentas con productos ingresados. Favor de ingresar los productos al sistema", this.#products)
        }
      ...
      return console.error("El archivo que desea revisar no existe");
    }
  }

  getProductsById(idQuery) {
    fs.readFile(this.#path, "utf-8", (err, data) => {
      if (err) {
        return console.error(
          "El archivo donde estan los productos a buscar no existe"
        );
      }
      ...
      if (productoSolicitado === undefined) {
        return console.error("No found it. El producto solicitado no existe");
      } else {
        return console.log(
          `El producto asociado al id ${idQuery} es:`,
          productoSolicitado
        );
      }
    });
